	...
	abstract class DB
	{
		public static function getAllMovies()
		{
			...
			$stmt = $connection->prepare("
				SELECT m.id AS 'movie_id', m.title, m.rating, m.awards, m.release_date, m.length,genre_id, g.name AS 'genre'
				FROM movies as m
		...
		public static function getActores() {
				...
				$consulta = $connection->prepare("
					SELECT actors.id, actors.first_name, actors.last_name, actors.rating, actors.favorite_movie_id ,movies.title as 'title'
					FROM actors
					INNER JOIN movies
          ...
          $actorObj->setId($actor["id"]);
          $actorObj->setRating($actor["rating"]);
          $actorObj->setPeliculaFavorita($actor["favorite_movie_id"]);
					$actorObj->setNombre($actor["first_name"]);
					$actorObj->setApellido($actor["last_name"]);
					$actorObj->setNombrePeliculaFavorita($actor["title"]);
        ...
        try {
					...
					$stmt->bindValue(':favorite_movie_id', $actor->getPeliculaFavorita());
